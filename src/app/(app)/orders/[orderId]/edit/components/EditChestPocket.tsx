...
export const EditChestPocketMeasurementInfo = ({
	...
	const chestValues = watch(
		`items.${orderItemIndexNumber}.measurement.chestPocket`,
	);
	...
	return (
		<CreateMeasurementSectionContainer>
			...
			<InputsGrid>
				<Controller
					control={control}
					name={`items.${orderItemIndexNumber}.measurement.chestPocketImg`}
				...
				{Object.keys(chestValues ?? {}).map((key) => {
					...
					if (unwrappedZod instanceof z.ZodEnum) {
						return (
							<Controller
								control={control}
								name={`items.${orderItemIndexNumber}.measurement.chestPocket.${key}`}
								key={`items.${orderItemIndexNumber}.measurement.chestPocket.${key}`}
					...
					const value = watch(
						`items.${orderItemIndexNumber}.measurement.chestPocket.${key}`,
					);
					return (
						<Controller
							key={`items.${orderItemIndexNumber}.measurement.chestPocket.${key}`}
